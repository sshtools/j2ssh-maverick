          ...
          try {
               ...
               if(ssh.isAuthenticated()) {
                ...
                if(remotefiles.length>2) {
                	int i=0;
                	while((remotefiles[i].getFilename().equals(".")|remotefiles[i].getFilename().equals(".."))&(i<remotefiles.length)) {
                		i++;
                	}
                	System.out.println("\n first remote filename"+remotefiles[i].getFilename());
                ...
                /**
                 * put all txt files in the local directory into the remote directory using *.txt
                 */
                sftp.putFiles("*.txt");
                System.out.println("\nPut *.txt\n");

...
/**
 * <p>All terminal sessions, forwarded connections, etc are channels and this class
 ...
 * controlled, no data may be sent to a channel until a message is received to indicate that
 * window space is available.</p>
 *
 * <p>To open a channel, first create an instance of the channel as follows:
 * <blockquote><pre>
 * Ssh2Channel channel = new Ssh2Channel("session", 32768, 32768);
 * </pre></blockquote>
 * The second value passed into the constructor is the initial window space and
 * also the maximum amount of window space that will be available to the other side.
 * The channel manages the window space automatically by increasing the amount
 * available once the data has been read from the channels InputStream and only
 * when the current value falls below half of the maximum.</p>
 *
 * <p>After the channel has been created the channel is opened through a call
 * to <a href="Ssh2Client.html#openChannel(com.maverick.ssh.message.SshAbstractChannel)">openChannel(Ssh2Channel channel)</a>.
 * Once the channel is open you can use the IO streams to send and receive data.</p>
 *
 *
...
public class Ssh2Channel
  ...
  /**
   * <p>Construct an SSH2 channel</p>
   * @param name the name of the channel, for example "session"
   * @param windowsize the initial window size
  ...
  /**
   * Called after the channel has been created by the <a href="ChannelFactory.html>ChannelFactory</a>.
   * There is no need to call this method directly, but it can be overridden to return data that should
   * be returned in the SSH_MSG_CHANNEL_OPEN_CONFIRMATION message.
   *
   * @return data that should be returned in the SSH_MSG_CHANNEL_OPEN_CONFIRMATION message
   * @throws IOException
   */
  ...
  protected void open(int remoteid,
                      long remotewindow,
                      int remotepacket) throws IOException {
    ...
    synchronized (listeners) {
      for (Enumeration<ChannelEventListener> e = listeners.elements(); e.hasMoreElements();) {
  ...
  /**
   * Once a SSH_MSG_CHANNEL_OPEN_CONFIRMATION message is received the framework calls this
   * method to complete the channel open operation.
   * @param remoteid the senders id
   * @param remotewindow the initial window space available for sending data
   * @param remotepacket the maximum packet size available for sending data
   * @param responsedata the data returned from the remote side in the SSH_MSG_CHANNEL_OPEN_CONFIRMATION message
   * @throws IOException
   */
  protected void open(int remoteid,
                      long remotewindow,
   ...
   * <a href="#channelRequest(java.lang.String, boolean, byte[])">channelRequest()</a>.
   */
  protected boolean processChannelMessage(SshChannelMessage msg) throws
      SshException {
    ...
    try {
      switch(msg.getMessageId()) {
        case SSH_MSG_CHANNEL_REQUEST:
          ...
          // #ifdef DEBUG
              EventLog.LogEvent(this,"Received SSH_MSG_CHANNEL_REQUEST id=" + channelid + " rid=" + remoteid + " request=" + requesttype + " wantreply=" + wantreply);
          // #endif
        ...
        case SSH_MSG_WINDOW_ADJUST:
            ...
            // #ifdef DEBUG
                ...
                EventLog.LogEvent(this,"Received SSH_MSG_WINDOW_ADJUST id=" + channelid + " rid=" + remoteid + " window=" + remotewindow.available() + " adjust=" + len);
                msg.reset();
            // #endif
        ...
        case SSH_MSG_CHANNEL_DATA:
            ...
            // #ifdef DEBUG
                EventLog.LogEvent(this,"Received SSH_MSG_CHANNEL_DATA id=" + channelid + " rid=" + remoteid + " len=" + (msg.available()-4) + " window=" + localwindow.available());
            // #endif
            ...
            if (autoConsumeInput) {
                ...
                if (localwindow.available() <= localwindow.getInitialSize() / 2) {
                    adjustWindow(localwindow.getInitialSize() - localwindow.available());
                }
            }

            for (Enumeration<ChannelEventListener> e = listeners.elements(); e.hasMoreElements(); ) {
                (e.nextElement()).dataReceived(
                        Ssh2Channel.this,
                        msg.array(),
                        msg.getPosition() + 4,
                        msg.available() - 4);
          ...
          case SSH_MSG_CHANNEL_EXTENDED_DATA:
              ...
              // #ifdef DEBUG
                  EventLog.LogEvent(this,"Received SSH_MSG_CHANNEL_EXTENDED_DATA id=" + channelid + " rid=" + remoteid + " len=" + (msg.available()-4) + " window=" + localwindow.available());
              // #endif
              ...
              for (Enumeration<ChannelEventListener> e = listeners.elements(); e.hasMoreElements(); ) {
                  (e.nextElement()).extendedDataReceived(
                          Ssh2Channel.this,
                          msg.array(),
                          msg.getPosition() + 8,
                          msg.available() - 8,
          ...
          case SSH_MSG_CHANNEL_CLOSE:
              ...
              // #ifdef DEBUG
                  EventLog.LogEvent(this,"Received SSH_MSG_CHANNEL_CLOSE id=" + channelid + " rid=" + remoteid);
              // #endif
          ...
          case SSH_MSG_CHANNEL_EOF:
              ...
              // #ifdef DEBUG
                  EventLog.LogEvent(this,"Received SSH_MSG_CHANNEL_EOF id=" + channelid + " rid=" + remoteid);
              // #endif
    ...
    switch (msg.getMessageId()) {
      ...
      case SSH_MSG_WINDOW_ADJUST:
        ...
        try {
            ...
            // #ifdef DEBUG
                EventLog.LogEvent(this,"Applied window adjust window=" + remotewindow.available());
              // #endif
        } catch(IOException ex) {
      ...
      case SSH_MSG_CHANNEL_EXTENDED_DATA:
        ...
        /**
         * We dont remove these messages because other threads may be evaluating the messages at the same
         * time
      */
      case SSH_MSG_CHANNEL_CLOSE:
        checkCloseStatus(true);
  ...
  /**
   * Called when channel data arrives, by default this method makes the
   * data available in the channels InputStream. Override this method to change
   * this behaviour.
   *
   * @param buf
   * @param offset
  ...
  protected void processStandardData(byte[] buf, int off, int len) throws SshException {
	  in.addMessage(len, new SshChannelMessage(SSH_MSG_CHANNEL_DATA, buf, off, len));
  }
   ...
   * event so to maintain code compatibility <em>always</em> call the
   * super method in any overidden method.
   *
   * @param typecode the type of extended data
  ...
  void sendChannelData(byte[] buf, int offset, int len) throws SshException {
    ...
    try {
      ...
      if(len > 0) {
        ...
        // #ifdef DEBUG
            EventLog.LogEvent(this,"Sending SSH_MSG_CHANNEL_DATA id=" + channelid + " rid=" + remoteid + " len=" + len + " window=" + remotewindow.available());
        // #endif
      ...
      try {
        ...
        if(listeners != null) {
          for(int i = 0; i < listeners.size(); i++) {
            ((ChannelEventListener)listeners.elementAt(i)).extendedDataReceived(Ssh2Channel.this,
               buf, offset, len, type);
          }
        }
      }
  ...
  private void adjustWindow(long increment) throws SshException {
	...
	try {
      ...
      // #ifdef DEBUG
          EventLog.LogEvent(this,"Sending SSH_MSG_WINDOW_ADJUST id=" + channelid
                   + " rid=" + remoteid + " window="
   ...
   * @param wantreply specifies whether the remote side should send a success/failure message
   * @param requestdata the request data
   * @return <code>true</code> if the request succeeded and wantreply=true, otherwise <code>false</code>
   * @throws IOException
   */
  public boolean sendRequest(String requesttype,
          boolean wantreply,
  ...
  synchronized(this) {
	  ...
	  try {
          ...
          // #ifdef DEBUG
          EventLog.LogEvent(this,"Sending SSH_MSG_CHANNEL_REQUEST id=" + channelid + " rid=" + remoteid + " request=" + requesttype + " wantreply=" + wantreply);
          // #endif
          connection.sendMessage(msg.toByteArray(), true);
  ...
  public void close() {
          ...
          if (performClose) {
              ...
              try {
                  ...
                  try {
                      // #ifdef DEBUG
                        EventLog.LogEvent(this,"Sending SSH_MSG_CHANNEL_CLOSE id=" + channelid + " rid=" + remoteid);
                      // #endif
                      connection.sendMessage(msg.toByteArray(), true);
                  } catch (SshException ex1) {
                	// #ifdef DEBUG
                      EventLog.LogEvent(this,"Exception attempting to send SSH_MSG_CHANNEL_CLOSE id=" + channelid + " rid=" + remoteid, ex1);
                      // #endif
                  } finally {
                	  msg.close();
                  }
            	...
            	// #ifdef DEBUG
                  EventLog.LogEvent(this,"SSH Exception during close reason=" + ex.getRealException().getReason() + " id=" + channelid + " rid=" + remoteid, ex.getRealException());
                  // #endif
                  // IO Error during close so the connection has dropped
                  connection.transport.disconnect(TransportProtocol.
                                                  CONNECTION_LOST,
                                                  "IOException during channel close: " +
                                                  ex.getMessage());
              } catch (IOException ex) {
                  // #ifdef DEBUG
                  EventLog.LogEvent(this,"Exception during close id=" + channelid + " rid=" + remoteid, ex);
                  // #endif
                  // IO Error during close so the connection has dropped
                  connection.transport.disconnect(TransportProtocol.
                                                  CONNECTION_LOST,
  ...
  /**
   * This channel is equal to another channel if the channel id's are equal. Please note
   * that channel numbers may be reused so this method should only be called on
   * open channels.
   */
  ...
  /**
   * Called when a channel request is received, by default this method sends a failure
   * message if the remote side requests a reply. Overidden methods should ALWAYS
   * call this superclass method.
   * @param requesttype the name of the request
   * @param wantreply specifies whether the remote side requires a success/failure message
   * @param requestdata the request data
   * @throws IOException
   */
...
///  long transferedOut = 0;
  class ChannelOutputStream
    ...
    public void close(boolean sendEOF) throws IOException {
      if (sendEOF) {
        ...
        try {
          // #ifdef DEBUG
              EventLog.LogEvent(this,"Sending SSH_MSG_CHANNEL_EOF id=" + getChannelId() + " rid=" + remoteid);
          // #endif
          connection.sendMessage(msg.toByteArray(), true);
        }
        catch(SshException ex) {
  ...
  class ChannelInputStream
    ...
    /**
     * This method returns the number of SSH messages that can be read using the read method without it blocking.
     * In certain circumstances there may be more messages available but this method cannot discover them without blocking and so returns 0. 
     */
   ...
   /* public long skip(long len) throws IOException {
    ...
    public synchronized int read(byte[] buf, int offset, int len) throws IOException {
      ...
      try {
    	...
    	/*if there is a message available then processMessages, 
    	  if its data then fills input buffer
    	  and sets unread
    	  */
        if(available() == -1) {
        	return -1;
        ...
        if (System.getProperty("maverick.windowAdjustTest", "false").equals("true") ||
        		(unread + localwindow.available()) < (localwindow.getInitialSize() / 2)
                && !isClosed() && !closing) {
          adjustWindow(localwindow.getInitialSize() - localwindow.available() - unread);
        }

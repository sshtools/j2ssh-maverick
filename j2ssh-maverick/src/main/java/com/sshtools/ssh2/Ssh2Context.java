...
/**
 * <p>This class implements <a href="../ssh/SshContext.html">SshContext</a>to provide SSH2 connection configuration
 * through the <a href="../ssh/SshConnector.html">SshConnector</a> class.</p>
 *
 * <p>To configure an authentication banner for SSH2 connections see <a href="BannerDisplay.html">BannerDisplay</a>.</p>
 *
 * <p>The preferred message authentication algorithm for each data stream can be set using:
 * <blockquote><pre>
 ...
 * </pre></blockquote>
 * Once further cipher, public key and compression algorithms have been implemented the same process
 * will apply.
 * </p>
...
public final class Ssh2Context
  ...
  public Ssh2Context() throws SshException {
    ...
    try {
      ...
      try {
        ...
        compressionsSC.add("zlib@openssh.com",
                Class.forName("com.sshtools.zlib.OpenSSHZLibCompression"));
      } catch (Throwable t) {}
      ...
      try {
    	  ...
    	  compressionsCS.add("zlib",
    			  Class.forName("com.sshtools.zlib.ZLibCompression"));
    	  compressionsCS.add("zlib@openssh.com",
    			  Class.forName("com.sshtools.zlib.OpenSSHZLibCompression"));
      ...
      throw new SshException(t.getMessage() != null ? t.getMessage() :
                            t.getClass().getName(),
                            SshException.INTERNAL_ERROR); // SSHException
    }
   ...
   * not normally require changing but some servers may support larger packets.
   * The default and minimum size is 35,000 bytes.
   *
   * @param maxPacketLength int
   */
   ...
   * @param bannerdisplay the banner display, may be null
   */
   ...
   * @param order, list of indices to be moved to the top.
 * @throws SshException
   */
 ...
 * @throws SshException
  ...
  public void enableCompression() throws SshException {
		  ...
		  prefCompressionCS = supportedCompressionsCS().changePositionofAlgorithm("none", 2);
		  ...
		  prefCompressionSC = supportedCompressionsSC().changePositionofAlgorithm("none", 2);
  ...
  public void disableCompression() throws SshException {
	  ...
	  prefCompressionCS = supportedCompressionsCS().changePositionofAlgorithm("zlib@openssh.com", 2);
	  
	  supportedCompressionsSC().changePositionofAlgorithm("none", 0);
	  supportedCompressionsSC().changePositionofAlgorithm("zlib", 1);
	  prefCompressionSC = supportedCompressionsSC().changePositionofAlgorithm("zlib@openssh.com", 2);
...
public void setKeyReExchangeDisabled(boolean keyReExchangeDisabled) {
   ...
   public void setKeyExchangePreferredPosition(String name,int position) throws SshException {
	   prefKeyExchange = keyExchanges.changePositionofAlgorithm(name, position);
   }
	  ...
	  public int getKeepAliveMaxDataLength() {
		...
		public void enableFIPSMode() throws SshException {
			...
			if(!keyExchanges.contains(Ssh2Context.KEX_DIFFIE_HELLMAN_GROUP14_SHA1)) {
				throw new SshException("Cannot enable FIPS mode because diffie-hellman-group14-sha1 " + 
						"keyexchange was not supported by this configuration. " + 
						"Install a JCE Provider that supports a prime size of 2048 bits (for example BouncyCastle provider)", SshException.BAD_API_USAGE);
			}
			...
			for(int i=0;i<names.length;i++) {
				if(!allowed.contains(names[i])) {
					EventLog.LogEvent(this, "Removing key exchange " + names[i]);
			...
			for(int i=0;i<names.length;i++) {
				if(!allowed.contains(names[i])) {
					EventLog.LogEvent(this, "Removing cipher client->server " + names[i]);
			...
			for(int i=0;i<names.length;i++) {
				if(!allowed.contains(names[i])) {
					EventLog.LogEvent(this, "Removing cipher server->client " + names[i]);
			...
			for(int i=0;i<names.length;i++) {
				if(!allowed.contains(names[i])) {
					EventLog.LogEvent(this, "Removing public key " + names[i]);
			...
			for(int i=0;i<names.length;i++) {
				if(!allowed.contains(names[i])) {
					EventLog.LogEvent(this, "Removing mac client->server " + names[i]);
			...
			for(int i=0;i<names.length;i++) {
				if(!allowed.contains(names[i])) {
					EventLog.LogEvent(this, "Removing mac server->client " + names[i]);

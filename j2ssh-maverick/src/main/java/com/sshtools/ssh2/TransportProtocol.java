...
/**
 * <p>Main implementation of the SSH Transport Protocol. The transport
 * is designed to run over a provider such as a Socket or StreamConnection.
 * To use first create an instance of the protocol and set its parameters and
 * when ready start the protocol using
 * <a href="#startTransportProtocol(TransportProvider)">startTransportProtocol</a>
 * supplying a <a href="TransportProvider.html">TransportProvider</a> instance.
 * This example uses the SocketProvider example implementation demonstrated
 * in <a href="TransportProvider.html">TransportProvider Help File</a>.
 * <blockquote><pre>
 ...
 * </pre></blockquote></p>
 *
 * <p>Host key verification is recommended and a callback interface is
 * provided by <a href="HostKeyVerification.html">HostKeyVerifcation</a>. To
     * force verification use <a href="#setHostKeyVerification(HostKeyVerification)">
 * setHostKeyVerification()</a> before starting the protocol. This is required
 * by default but can be ignored with <a href="#ignoreHostKeyVerification(boolean)">
 * ignoreHostKeyVerification(boolean ignore)</a>.</p>
 *
 * <p>Additional cipher, message authentication and compression components can
 * be supported by providing a custom <a href="TransportContext.html">
     * TransportContext</a>. The default context provides all the required components
 * of the protocol but others can be added by creating an instance and adding
 * the various implementations to the component factories.
 * <blockquote><pre>
 ...
 *                                          context);
 * </pre></blockquote></p>
 * <p>The context can also be used to specify the preferred methods of
 * encryption for the connection which you may want to use should you
...
public class TransportProtocol
  ...
  /**
   * Character set encoding. All input/output strings created by the
   * API are created with this encoding. The default is "UTF-8"
   * and it may be changed, the results however are unexpected.
   */
  ...
  /**
   * Protocol state: The transport protocol is connected and services can
   * be started or may already be active.
   */
   ...
   * Returns the connected state
       * @return <tt>true</tt> if the transport is connected, otherwise <tt>false</tt>
   */
  public boolean isConnected() {
    return currentState == CONNECTED || currentState == PERFORMING_KEYEXCHANGE;
  }

  /**
   * Returns the last error detected by the protocol. If a disconnect
   * occurs this may provide a reason.
  ...
  public void startTransportProtocol(SshTransport provider,
    ...
    try {
      ...
      this.incomingMessage = new byte[transportContext.getMaximumPacketLength()];
      this.outgoingMessage = new ByteArrayWriter(transportContext.getMaximumPacketLength());
      this.client = client;
   ...
   * @param reason
   * @param disconnectReason, description
   * @throws IOException
   */
  public void disconnect(int reason, String disconnectReason) {
	...
	try {
      ...
      EventLog.LogEvent(this,"Sending SSH_MSG_DISCONNECT [" + disconnectReason + "]");
  ...
  /**
   * <p>Send a transport protocol message. The format of the message should
   * be:
   * <blockquote><pre>
   * byte        Message ID
   * byte[]      Payload
  ...
  public void sendMessage(byte[] msgdata, boolean isActivity) throws SshException {
    ...
    synchronized(kexqueue) {
		...
		if (currentState == PERFORMING_KEYEXCHANGE && !isTransportMessage(msgdata[0])) {
     ...
     try {
        ...
        padding += ((outgoingCipherLength
                     - ((msgdata.length + 5 + padding) % outgoingCipherLength)) %
                    outgoingCipherLength);
        ...
        ComponentManager.getInstance().getRND().nextBytes(outgoingMessage.array(), outgoingMessage.size(), padding);
        ...
        // Perfrom encrpytion
        if(encryption != null) {
          encryption.transform(outgoingMessage.array(),
                               0,
                               outgoingMessage.array(),
                               0,
                               outgoingMessage.size());
        ...
        // Send!
        transportOut.write(outgoingMessage.array(),
                           0,
                           outgoingMessage.size());
        transportOut.flush();
        ...
        // #ifdef DEBUG
		if(verbose) {
          EventLog.LogDebugEvent(this,"Sent " + outgoingMessage.size() + " bytes of transport data outgoingSequence="
             + outgoingSequence + " totalBytesSinceKEX=" + numOutgoingBytesSinceKEX);
		}
        // #endif
        ...
        internalDisconnect();
        throw new SshException("Unexpected termination: " + ex.getMessage(),
   ...
   * so skipping the first 5 bytes is required before the message data
   * can be read.
  ...
  void readWithTimeout(byte[] buf, int off, int len, int timeoutMillis, boolean isPartialMessage) throws SshException {
      ...
      if(isPartialMessage) {
    	  ...
    	  timeout = configureSocketTimeout(transportContext.getPartialMessageTimeout());
      }
      ...
      try {
          do {
              try {
                  ...
                  //#ifdef DEBUG
                  EventLog.LogEvent(this,"Socket timed out during read! "
                       + " isPartialMessage=" + isPartialMessage + " bytesTransfered=" + ex.bytesTransferred);
                  //#endif
                  ...
                  if(isPartialMessage) {
                	  ...
                	  throw new SshException(
						 ...
						 if(getContext().getIdleConnectionTimeoutSeconds() > 0 && 
								 (System.currentTimeMillis() - lastActivity) > (getContext().getIdleConnectionTimeoutSeconds() * 1000)) {
							 
							//#ifdef DEBUG
							 EventLog.LogEvent(this,"Connection is idle, disconnecting idleMax=" + getContext().getIdleConnectionTimeoutSeconds());
							//#endif
							 disconnect(TransportProtocol.BY_APPLICATION, "Idle connection");
							 throw new SshException(
										"Connection has been dropped as it reached max idle time of " + getContext().getIdleConnectionTimeoutSeconds() + " seconds.",
							 ...
							 try {
								...
								//#endif
								 ...
								 int tmplen = (int) (Math.random() * (getContext().getKeepAliveMaxDataLength()) + 1);
								 ...
								 // Disconnected
								 internalDisconnect("Connection failed during SSH_MSG_IGNORE packet", CONNECTION_LOST);
							 } finally {
								 try {
									baw.close();
								} catch (IOException e) {
						 ...
						 if(getContext().getSocketTimeout() > 0) {
							 ...
							 for(Enumeration<TransportProtocolListener> e = listeners.elements(); e.hasMoreElements();) {
				...
				/**throw new SshException(
                  ...
                  throw new SshException(
                          "IO error received from remote"+ex.getMessage(),
  ...
  byte[] readMessage() throws SshException {
    ...
    synchronized (transportIn) {
      ...
      try {
        ...
        readWithTimeout(incomingMessage, 0, incomingCipherLength, transportContext.getPartialMessageTimeout(), false);

//	  Decrypt the data if we have a valid cipher
        if(decryption != null) {
          decryption.transform(incomingMessage, 0, incomingMessage, 0,
...
//	  Preview the message length
        ...
        if(msglen <= 0)
                throw new SshException("Server sent invalid message length of " + msglen + "!", SshException.PROTOCOL_VIOLATION);
        ...
        // #ifdef DEBUG
        if(verbose) {  
        	EventLog.LogEvent(this,"Incoming transport message msglen=" + msglen + " padlen=" + padlen);
        }
        // #endif
...
//      Verify that the packet length is good
        if(remaining < 0) {
          ...
          throw new SshException(
           ...
           SshException.UNEXPECTED_TERMINATION);
        } else if(remaining > incomingMessage.length - incomingCipherLength) {

          if(remaining + incomingCipherLength + incomingMacLength > transportContext.getMaximumPacketLength()) {
              internalDisconnect();
              throw new SshException(
                         "Incoming packet length violates SSH protocol [" + remaining + incomingCipherLength + " bytes]",
                         SshException.UNEXPECTED_TERMINATION);
          }
          // Resize the incomingMessage buffer
          byte[] tmp = new byte[remaining + incomingCipherLength + incomingMacLength];
          System.arraycopy(incomingMessage, 0, tmp, 0, incomingCipherLength);
          incomingMessage = tmp;
...
//	  Read, decrypt and save the remaining data
        if(remaining > 0) {
            ...
            readWithTimeout(incomingMessage, incomingCipherLength, remaining, transportContext.getPartialMessageTimeout(), true);

            if (decryption != null) {
                decryption.transform(incomingMessage, incomingCipherLength,
                                     incomingMessage, incomingCipherLength, remaining);
            }
//	  Verify the message
        }
        if(incomingMac != null) {
          readWithTimeout(incomingMessage,
                                incomingCipherLength + remaining,
                                incomingMacLength, transportContext.getPartialMessageTimeout(), true);
        ...
        return payload;
          ...
          throw new SshException("Interrupted IO; possible socket timeout detected?",
                                 SshException.SOCKET_TIMEOUT);
      }
      catch(IOException ex) {
        internalDisconnect();
        throw new SshException("Unexpected terminaton: "
                               ...
                               + " sequenceNo = " + incomingSequence
                               + " bytesIn = " + incomingBytes
  ...
  void performKeyExchange(byte[] msg) throws SshException {
	...
	ByteArrayReader bar = new ByteArrayReader(msg, 0,
              msg.length);
    try {
      synchronized(kexqueue) {

        // If were not already in a key exchange state then send our kex init
        if(localkex == null) {
          sendKeyExchangeInit(false);
        }

        // Set the state to performing key exchange now that we have both msgs
        currentState = TransportProtocol.PERFORMING_KEYEXCHANGE;

        // Extract the remote's side kex init taking away the header and padding
        remotekex = msg;

        bar.skip(17);

        String remoteKeyExchanges = checkValidString("key exchange", bar.readString());
        String remotePublicKeys = checkValidString("public key", bar.readString());
        String remoteCiphersCS = checkValidString("client->server cipher", bar.readString());
        String remoteCiphersSC = checkValidString("server->client cipher", bar.readString());
        String serverCSMacs= checkValidString("client->server mac", bar.readString());
        String serverSCMacs= checkValidString("server->client mac", bar.readString());
        String serverCSCompressions= checkValidString("client->server comp", bar.readString());
        String serverSCCompressions= checkValidString("server->client comp", bar.readString());
        ...
        EventServiceImplementation.getInstance().fireEvent((new Event(this,J2SSHEventCodes.EVENT_KEY_EXCHANGE_INIT,true))
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_REMOTE_KEY_EXCHANGES, remoteKeyExchanges)
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_LOCAL_KEY_EXCHANGES, transportContext.supportedKeyExchanges().list(transportContext.getPreferredKeyExchange()))
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_REMOTE_PUBLICKEYS, remotePublicKeys)
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_LOCAL_PUBLICKEYS, transportContext.supportedPublicKeys().list(transportContext.getPreferredPublicKey()))
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_REMOTE_CIPHERS_CS, remoteCiphersCS)
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_LOCAL_CIPHERS_CS, transportContext.supportedCiphersCS().list(transportContext.getPreferredCipherCS()))
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_REMOTE_CIPHERS_SC, remoteCiphersSC)
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_LOCAL_CIPHERS_SC, transportContext.supportedCiphersSC().list(transportContext.getPreferredCipherSC()))
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_REMOTE_CS_MACS, serverCSMacs)
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_LOCAL_CS_MACS, transportContext.supportedMacsCS().list(transportContext.getPreferredMacCS()))
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_REMOTE_SC_MACS, serverSCMacs)
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_LOCAL_SC_MACS, transportContext.supportedMacsSC().list(transportContext.getPreferredMacSC()))
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_REMOTE_CS_COMPRESSIONS, serverCSCompressions)
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_LOCAL_CS_COMPRESSIONS, transportContext.supportedCompressionsCS().list(transportContext.getPreferredCompressionCS()))
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_REMOTE_SC_COMPRESSIONS, serverSCCompressions)
      		  .addAttribute(J2SSHEventCodes.ATTRIBUTE_LOCAL_SC_COMPRESSIONS, transportContext.supportedCompressionsSC().list(transportContext.getPreferredCompressionSC()))
      		  );

        // #ifdef DEBUG
          EventLog.LogDebugEvent(this,"Remote computer supports key exchanges: " + remoteKeyExchanges);
        // #endif

        // #ifdef DEBUG
          EventLog.LogDebugEvent(this,"Remote computer supports public keys: " + remotePublicKeys);
        // #endif

        // #ifdef DEBUG
          EventLog.LogDebugEvent(this,"Remote computer supports client->server ciphers: " + remoteCiphersCS);
        // #endif

        String cipherCS = selectNegotiatedComponent(
           transportContext.supportedCiphersCS().list(transportContext.getPreferredCipherCS()),
           remoteCiphersCS);

        // #ifdef DEBUG
          EventLog.LogDebugEvent(this,"Negotiated client->server cipher: " + cipherCS);
        // #endif
        ...
        String cipherSC = selectNegotiatedComponent(
           transportContext.supportedCiphersSC().list(transportContext.getPreferredCipherSC()),
           remoteCiphersSC);

        // #ifdef DEBUG
            EventLog.LogDebugEvent(this,"Negotiated server->client cipher: " + cipherSC);
        // #endif

        SshCipher encryption = (SshCipher)transportContext.supportedCiphersCS().getInstance(cipherCS);

        SshCipher decryption = (SshCipher)transportContext.supportedCiphersSC().getInstance(cipherSC);
        String macCS = selectNegotiatedComponent(
           transportContext.supportedMacsCS().list(transportContext.getPreferredMacCS()),
           checkValidString("client->server hmac", serverCSMacs));

        String macSC = selectNegotiatedComponent(
           transportContext.supportedMacsSC().list(transportContext.getPreferredMacSC()),
           checkValidString("server->client hmac", serverSCMacs));

        SshHmac outgoingMac = (SshHmac)transportContext.supportedMacsCS().
           getInstance(macCS);

        SshHmac incomingMac = (SshHmac)transportContext.supportedMacsSC().
           getInstance(macSC);

        String compressionCS = selectNegotiatedComponent(
           transportContext.supportedCompressionsCS().list(transportContext.getPreferredCompressionCS()),
           checkValidString("client->server compression", serverCSCompressions));

        String compressionSC = selectNegotiatedComponent(
           transportContext.supportedCompressionsSC().list(transportContext.getPreferredCompressionSC()),
           checkValidString("server->client compression", serverSCCompressions));

        SshCompression outgoingCompression = null;

        if (!compressionCS.equals(Ssh2Context.COMPRESSION_NONE)) {
        	outgoingCompression=(SshCompression)transportContext.supportedCompressionsCS().getInstance(compressionCS);
        ...
        if (!compressionSC.equals(Ssh2Context.COMPRESSION_NONE)) {
        	incomingCompression=(SshCompression)transportContext.supportedCompressionsSC().getInstance(compressionSC);
        	incomingCompression.init(SshCompression.INFLATER, 6);
        }

        boolean ignoreFirstPacket = false;
        
        String keyExchangeAlg = selectNegotiatedComponent(transportContext.supportedKeyExchanges().list(transportContext.getPreferredKeyExchange()),remoteKeyExchanges);
        
        if(guessedKeyExchange==null || !keyExchangeAlg.equals(guessedKeyExchange.getAlgorithm())) {
	        // Determine the negotiated key exchange
	        keyExchange = (SshKeyExchangeClient)transportContext.supportedKeyExchanges().getInstance(keyExchangeAlg);
        }
        
        // #ifdef DEBUG
           EventLog.LogDebugEvent(this,"Negotiated key exchange: " + keyExchange.getAlgorithm());
        // #endif
           
        
        if(guessed) {
           	// Should we ignore the first key exchange packet?
           	if(!keyExchangeAlg.equals(transportContext.getPreferredKeyExchange())) {
           		ignoreFirstPacket = true;
           	}
       		String hostkey = selectNegotiatedComponent(
       				transportContext.supportedPublicKeys().list(
       						transportContext.getPreferredPublicKey()), remotePublicKeys);
       		
       		if(!ignoreFirstPacket && !hostkey.equals(transportContext.getPreferredPublicKey())) {
        ...
        String hostKeyAlg=selectNegotiatedComponent(transportContext.supportedPublicKeys().list(transportContext.getPreferredPublicKey()),remotePublicKeys); 
        hostkey = (SshPublicKey)transportContext.supportedPublicKeys().getInstance(hostKeyAlg);

        if(!(ignoreHostKeyifEmpty && Arrayequals(keyExchange.getHostKey(),"".getBytes()))) {

        	EventServiceImplementation.getInstance().fireEvent((new Event(this,J2SSHEventCodes.EVENT_HOSTKEY_RECEIVED,true)).addAttribute(J2SSHEventCodes.ATTRIBUTE_HOST_KEY, new String(keyExchange.getHostKey())));
	        hostkey.init(keyExchange.getHostKey(),
	                 0,
	                 keyExchange.getHostKey().length);

	        if(transportContext.getHostKeyVerification() != null) {
	          if(!transportContext.getHostKeyVerification().verifyHost(provider.getHost(), hostkey)) {
	        	EventServiceImplementation.getInstance().fireEvent(new Event(this,J2SSHEventCodes.EVENT_HOSTKEY_REJECTED,false));
	        	disconnect(TransportProtocol.HOST_KEY_NOT_VERIFIABLE,"Host key not accepted");
	            throw new SshException("The host key was not accepted",SshException.CANCELLED_CONNECTION);
	          }

	          if(!hostkey.verifySignature(keyExchange.getSignature(),keyExchange.getExchangeHash())) {
	        	EventServiceImplementation.getInstance().fireEvent(new Event(this,J2SSHEventCodes.EVENT_HOSTKEY_REJECTED,false));
	            disconnect(TransportProtocol.HOST_KEY_NOT_VERIFIABLE,"Invalid host key signature");
	            throw new SshException("The host key signature is invalid",SshException.PROTOCOL_VIOLATION);
	          }
	          EventServiceImplementation.getInstance().fireEvent(new Event(this,J2SSHEventCodes.EVENT_HOSTKEY_ACCEPTED,true));
	        }
      	}
        ...
        // so lets send our new keys message and wait for the other sides
        // response
        sendMessage(new byte[] {(byte)SSH_MSG_NEWKEYS}, true);

        // Put the outgoing components into use
        encryption.init(SshCipher.ENCRYPT_MODE,
                        makeSshKey('A'),
                        makeSshKey('C'));
        ...
        do {
          ...
          if(!processMessage(msg)) {
        	  EventServiceImplementation.getInstance().fireEvent(new Event(this,J2SSHEventCodes.EVENT_KEY_EXCHANGE_FAILURE,true));
        ...
        EventServiceImplementation.getInstance().fireEvent((new Event(this,J2SSHEventCodes.EVENT_KEY_EXCHANGE_COMPLETE,true))
        			.addAttribute(J2SSHEventCodes.ATTRIBUTE_USING_PUBLICKEY, hostKeyAlg)
        			.addAttribute(J2SSHEventCodes.ATTRIBUTE_USING_KEY_EXCHANGE, keyExchangeAlg)
        			.addAttribute(J2SSHEventCodes.ATTRIBUTE_USING_CS_CIPHER, cipherCS)
        			.addAttribute(J2SSHEventCodes.ATTRIBUTE_USING_SC_CIPHER, cipherSC)
        			.addAttribute(J2SSHEventCodes.ATTRIBUTE_USING_CS_MAC, macCS)
        			.addAttribute(J2SSHEventCodes.ATTRIBUTE_USING_SC_MAC, macSC)
        			.addAttribute(J2SSHEventCodes.ATTRIBUTE_USING_CS_COMPRESSION, compressionCS)
        			.addAttribute(J2SSHEventCodes.ATTRIBUTE_USING_SC_COMPRESSION, compressionSC)
        			);
                  


        // Put the incoming components into use
        decryption.init(SshCipher.DECRYPT_MODE,
                        makeSshKey('B'),
                        makeSshKey('D'));
       ...
       // synchronized(kexqueue) {
          ...
          for(Enumeration<byte[]> e = kexqueue.elements(); e.hasMoreElements(); ) {
   ...
   * This is only available in CLIENT_MODE.
   * @param servicename
   * @throws IOException
   */
   ...
   * outside of the transport protocol so that the transport protocol
   * can parse its own messages.
   * @param msg
    ...
    try {
      ...
      switch(msg[0]) {
        case SSH_MSG_DISCONNECT: {
          ...
          try {
	          EventServiceImplementation.getInstance().fireEvent(new Event(this,J2SSHEventCodes.EVENT_RECEIVED_DISCONNECT,true));
	          throw new SshException(bar.readString(),SshException.REMOTE_HOST_DISCONNECTED);
          } finally {
        	  bar.close();
          }
        }
  ...
  String selectNegotiatedComponent(String locallist, String remotelist) throws
      SshException {
    ...
    EventServiceImplementation.getInstance().fireEvent((new Event(this,J2SSHEventCodes.EVENT_FAILED_TO_NEGOTIATE_TRANSPORT_COMPONENT,true)).addAttribute(J2SSHEventCodes.ATTRIBUTE_LOCAL_COMPONENT_LIST, locallist).addAttribute(J2SSHEventCodes.ATTRIBUTE_REMOTE_COMPONENT_LIST, remotelist));
  ...
  void sendKeyExchangeInit(boolean guess) throws SshException {
	...
	try {
        synchronized(kexqueue) {
	      ...
	      msg.writeString(transportContext.supportedCiphersCS().list(transportContext.getPreferredCipherCS()));
	      msg.writeString(transportContext.supportedCiphersSC().list(transportContext.getPreferredCipherSC()));
	      msg.writeString(transportContext.supportedMacsCS().list(transportContext.getPreferredMacCS()));
	      msg.writeString(transportContext.supportedMacsSC().list(transportContext.getPreferredMacSC()));
	      msg.writeString(transportContext.supportedCompressionsCS().list(
	    	 transportContext.getPreferredCompressionCS()));
	      msg.writeString(transportContext.supportedCompressionsSC().list(
  ...
  byte[] makeSshKey(char chr) throws IOException {
	...
	try {
		...
		Digest hash = (Digest) ComponentManager.getInstance().supportedDigests().getInstance(keyExchange.getHashAlgorithm());

...
/**
 * Provides hostbased authentication for the SSH2 protocol. Hostbased authentication
 * allows a user to connect from a trusted client by providing the clients
 * public key and their local/remote usernames. The server then allows access
 * if the client can be verified through a combination of several different
 * configuration files which include /etc/hosts.equiv /etc/ssh/ssh_known_hosts
 * ~/.ssh/known_hosts ~/.rhosts ~./shosts.
 *
  ...
  public void authenticate(AuthenticationProtocol authentication, String servicename) throws SshException, AuthenticationResult {
    ...
    try {
      ...
      throw new SshException(
         "Unexpected message returned from authentication protocol: " + reply[0],

...
/**
 * An implementation of the diffie-hellman-group-exchange key exchange mechanism that
 * uses JCE provider for DH agreement and Digest.
 *
 * @author Lee David Painter
 */
public class DiffieHellmanGroupExchangeSha1
    extends SshKeyExchangeClient implements AbstractKeyExchange {
  ...
  public void performClientExchange(String clientIdentification,
    ...
    try {
      ...
      if(tmp[0]!=SSH_MSG_KEXDH_GEX_GROUP) {
        ...
        throw new SshException("Key exchange failed: Expected SSH_MSG_KEX_GEX_GROUP [id=" + tmp[0] + "]",
     ...
     try {
      ...
      if(tmp[0] != SSH_MSG_KEXDH_GEX_REPLY) {
        ...
        throw new SshException("Key exchange failed: Expected SSH_MSG_KEXDH_GEX_REPLY [id=" + tmp[0] + "]",
                               SshException.INTERNAL_ERROR);
      }
  ...
  protected void calculateExchangeHash(boolean disableBackwardsCompatibility, int preferredKeySize) throws SshException {
    ...
    if(disableBackwardsCompatibility) {
    	...
    	hash.putInt(8192); // This breaks some old servers, use backwards compatibility
    } else {
    	hash.putInt(preferredKeySize);
    }

...
/**
 * A component manager for the Java runtime JCE provider. By default all algorithms
 * will be selected from the default provider i.e no provider is specified in calls
 * to JCE methods to create components.  You can initialize a default provider
 * to be used on all calls with the following code:
 * 
 * <blockquote><pre>
 ...
 * algorithm, all algorithms used by the API are included as static constants
 * in this class.
 * 
 * <blockquote><pre>
...
public class JCEComponentManager extends ComponentManager implements JCEAlgorithms {
	...
	public JCEComponentManager() {
		  ...
		  try {
				...
				JCEComponentManager.initializeProviderForAlgorithm(JCEAlgorithms.JCE_DH, bc);
		  } catch(Throwable t) { 
			  EventLog.LogErrorEvent(this, "Could not find BouncyCastle provider diffie-hellman-group14-sha1 may not be available");
		  }
	...
	public SshRsaPrivateCrtKey createRsaPrivateCrtKey(BigInteger modulus,
			...
			BigInteger primeP, BigInteger primeQ, BigInteger crtCoefficient) throws SshException {
		...
		try {
			...
			return new Ssh2RsaPrivateCrtKey(modulus, publicExponent, privateExponent,
	...
	public SshKeyPair generateDsaKeyPair(int bits) throws SshException {
		...
		try {
	        ...
	        KeyPairGenerator keyGen = JCEProvider.getProviderForAlgorithm(JCE_DSA)==null ?
	        		KeyPairGenerator.getInstance(JCE_DSA) : KeyPairGenerator.getInstance(JCE_DSA, JCEProvider.getProviderForAlgorithm(JCE_DSA));
	...
	public SshKeyPair generateRsaKeyPair(int bits) throws SshException {
		try {
			...
			KeyPairGenerator keyGen = JCEProvider.getProviderForAlgorithm(JCE_RSA)==null ?
	        		KeyPairGenerator.getInstance(JCE_RSA) : KeyPairGenerator.getInstance(JCE_RSA, JCEProvider.getProviderForAlgorithm(JCE_RSA));
	        keyGen.initialize(bits);
	        KeyPair keypair = keyGen.genKeyPair();
	        PrivateKey privateKey = keypair.getPrivate();
	        PublicKey publicKey = keypair.getPublic();
	        ...
	        if(!(privateKey instanceof RSAPrivateCrtKey)) {
	        	throw new SshException("RSA key generation requires RSAPrivateCrtKey as private key type.", SshException.JCE_ERROR);
	        }
...
//		}
	...
	protected void initializeKeyExchangeFactory(ComponentFactory keyexchange) {
		//sshd has its own version of these classes so they will not be on its classpath, this is why we use class.forname
		try {
			Class<?> DiffieHellmanGroup14Sha1=Class.forName("com.maverick.ssh.components.jce.DiffieHellmanGroup14Sha1");
			Class<?> DiffieHellmanGroup1Sha1=Class.forName("com.maverick.ssh.components.jce.DiffieHellmanGroup1Sha1");
			Class<?> DiffieHellmanGroupExchangeSha1=Class.forName("com.maverick.ssh.components.jce.DiffieHellmanGroupExchangeSha1");
			Class<?> DiffieHellmanGroupExchangeSha256=Class.forName("com.maverick.ssh.components.jce.DiffieHellmanGroupExchangeSha256");
			
			if(testKeyExchangeAlgorithm("diffie-hellman-group14-sha1", DiffieHellmanGroup14Sha1)) {
				keyexchange.add("diffie-hellman-group14-sha1", DiffieHellmanGroup14Sha1);
			}
			
			if(testKeyExchangeAlgorithm("diffie-hellman-group1-sha1", DiffieHellmanGroup1Sha1)) {
				keyexchange.add("diffie-hellman-group1-sha1", DiffieHellmanGroup1Sha1);
			}
	
			if(testKeyExchangeAlgorithm("diffie-hellman-group-exchange-sha1", DiffieHellmanGroupExchangeSha1)) {
				keyexchange.add("diffie-hellman-group-exchange-sha1", DiffieHellmanGroupExchangeSha1);
			}
			
			if(testKeyExchangeAlgorithm("diffie-hellman-group-exchange-sha256", DiffieHellmanGroupExchangeSha256)) {
				keyexchange.add("diffie-hellman-group-exchange-sha256", DiffieHellmanGroupExchangeSha256);
			}
	...
	protected void initializePublicKeyFactory(ComponentFactory publickeys) {
		...
		publickeys.add(SshX509RsaPublicKey.X509V3_SIGN_RSA, SshX509RsaPublicKey.class);
		publickeys.add(SshX509DsaPublicKey.X509V3_SIGN_DSA, SshX509DsaPublicKey.class);
		publickeys.add(SshX509RsaSha1PublicKey.X509V3_SIGN_RSA_SHA1, SshX509RsaSha1PublicKey.class);
	}
	...
	private boolean testKeyExchangeAlgorithm(String name, Class<?> keyExchangeAlgorithmClass) {
		...
		try {
			...
			//a null pointer exception will be caught at the end of the keyex call when transport.sendmessage is called, at this point the algorithm has not thrown an exception so we ignore this excpected exception.
		}
		
		try {
			Method test=keyExchangeAlgorithmClass.getMethod("getProvider", new Class[] {} );
			provider = (String) test.invoke(SshKeyExchangeClient_Instance, new Object[] {} );
		} catch(Throwable t) { }
		
		EventLog.LogEvent(this, "   " + name + " will be supported using JCEProvider " + provider);
		return true;
	}
	...
	private boolean testJCECipher(String name, Class<?> cls) {
		try {
			...
			if(c instanceof AbstractJCECipher)
				EventLog.LogEvent(this, "   " + name + " will be supported using JCE Provider " + ((AbstractJCECipher)c).getProvider());
			
			return true;
		} catch (Throwable e) {
			EventLog.LogEvent(this, "   " + name + " will not be supported: " + e.getMessage());
			return false;
		}
	}
	...
	private boolean testDigest(String name, Class<?> cls) {
		try {
			...
			if(c instanceof AbstractDigest)
				EventLog.LogEvent(this, "   " + name + " will be supported using JCE Provider " + ((AbstractDigest)c).getProvider());
	...
	private boolean testHMac(String name, Class<?> cls) {
		try {
			...
			if(c instanceof AbstractHmac)
				EventLog.LogEvent(this, "   " + name + " will be supported using JCE Provider " + ((AbstractHmac)c).getProvider());

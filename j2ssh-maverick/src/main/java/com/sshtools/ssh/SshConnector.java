...
/**
 * <p>This utility class establishes
 * a connection with an SSH server, determines which SSH protocol versions are supported
 * and creates an initialized connection ready for authentication.<p>
 *
 * <p>Each call to <a href="#createInstance()">createInstance()</a> returns a new instance
 * of the connector with a configuration context. These are designed to be re-used for many
 * connections where the same configuration can be used.<p>
 *
 * <p>To connect to an SSH server you need to provide an <a href="SshTransport.html">SshTransport</a>
 * which provides the transport layer communication. In most cases this will be a Socket however since
 * this API is designed for many different platforms, a Socket may not always be available so this simple
 * interface is used to allow you to specify the source IO streams. The <a href="SshTransport.html">SshTransport</a>
 * documentation provides a simple example for a Socket called <a href="SshTransport#SocketTransport">SocketTransport</a>
 * which is used in the following examples.</p>
 *
 * <p>To create a connection and authentication using password authentication with default configuration
 * contexts use:
 * <blockquote><pre>
...
public final class SshConnector {
  ...
  /**
   * Returns an instance of the <code>SshConnector</code>. Each instance is initialized
  ...
  /**
   * <p>Get the configuration context.
   *
  ...
  /**
   * <p>Create a new connection to an SSH server over the specified transport. This
   ...
   * server the connector will always operate using SSH2.<p>
   *
   * <p>The {@link SshTransport} interface is used here to allow different types
   ...
   * {@link SshTransport} documentation for an example Socket implementation.<p>
   *
   *
   * @param transport  the transport for the connection.
   * @param username   the name of the user connecting
   * @return a connected <a href="SshClient.html">SshClient</a> instance ready for authentication.
   ...
   * helps in circumstances where you require a channel to fill up with data without
   * calling its InputStream's read method. This also will enable the InputStreams
   * available method to work as expected.</p>
   *
   * <p>This method also allows you to specify a context to use. Normally you would
   * reuse an {@link SshConnector} instead of calling this method directly.</p>
   *
   * @param transport SshTransport
   * @param username String
   * @param buffered boolean
  ...
  public SshClient connect(SshTransport transport,
  	...
  	//#ifdef DEBUG
  	EventLog.LogEvent(this,"Connecting " + username + "@" + transport.getHost()
 ...
 // Lets first try SSH2 cause its a better protocol
	...
	if(ctx.getSocketTimeout() > 0 && transport instanceof SocketTimeoutSupport) {
		try {
			((SocketTimeoutSupport)transport).setSoTimeout(ctx.getSocketTimeout());
		} catch (IOException e) {
	...
	//#ifdef DEBUG
	EventLog.LogEvent(this,"Attempting to determine remote version");
	//#endif
    ...
    try {
       ...
       //#ifdef DEBUG
       EventLog.LogEvent(this,"Attempting SSH2 connection");
       //#endif
       ...
       client = new Ssh2Client();
   	  ...
   	  //#ifdef DEBUG
      EventLog.LogEvent(this,"Remote identification: " + remoteIdentification);
      EventLog.LogEvent(this,"Local identification: " + localIdentification.trim() + " [" + originalSoftwareComments + "]");
      //#endif
      
      client.connect(transport, ssh2Context == null ? context : ssh2Context, this,
                     username,
      ...
      return client;
		...
		throw new SshException(t.getMessage() != null ?
                                 t.getMessage() :
                                 t.getClass().getName(),
  ...
  String getRemoteIdentification(SshTransport transport) throws SshException {
    ...
    try {
      ...
      while(!remoteIdentification.startsWith("SSH-")) {
        ...
        while(((ch = in.read()) != '\n')
              && (lineBuffer.length() < MAX_BUFFER_LENGTH)
              && ch > -1) {
          if(ch== '\r') {
              continue;
          }
        ...
        if(ch == -1) {
          throw new SshException(
             "Failed to read remote identification "+lineBuffer.toString(),
             SshException.CONNECT_FAILED);
        }

 ...
 * 
 * <p>
 * This class processes text data and corrects line endings according to the
...
public class EOLProcessor {
	...
	public EOLProcessor(int inputStyle, int outputStyle, OutputStream out)
			throws IOException {
		...
		default: {
			throw new IllegalArgumentException("Unknown text style: "
					+ outputStyle);
		}
		}
		...
		case TEXT_ALL: {
			throw new IllegalArgumentException(
					"TEXT_ALL cannot be used for an output style");
		}
		default: {
			throw new IllegalArgumentException("Unknown text style: "
					+ outputStyle);
		...
		while ((b = bin.read()) != -1) {
			...
			if (b == '\r') {
				...
				if (stripCRLF) {
					...
					if (ch == '\n') {
						// This is STYLE_RN do we output as is or replace with
						// NL?
						out.write(lineEnding);
				...
				// Are we processing between blocks and was the last character a
				// CR
				if (lastCharacterWasCR) {
					out.write(lineEnding);
	...
	public static OutputStream createOutputStream(int inputStyle,
			int outputStyle, OutputStream out) throws IOException {
		return new EOLProcessorOutputStream(inputStyle, outputStyle, out);
	}

	public static InputStream createInputStream(int inputStyle,
			int outputStyle, InputStream in) throws IOException {

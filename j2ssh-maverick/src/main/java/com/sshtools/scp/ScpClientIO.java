...
public class ScpClientIO {
    ...
    public void put(InputStream in, long length, String localFile, String remoteFile) throws SshException, ChannelOpenException {
        put(in, length, localFile, remoteFile, null);
    }
     ...
     * @param localFile local file name
     * @param remoteFile remote file name
     * @param progress a file transfer progress implementation
     * 
     * @throws IOException on any error
     */
    public void put(InputStream in, long length, String localFile, String remoteFile, FileTransferProgress progress)
                    throws SshException, ChannelOpenException {

        ScpEngineIO scp = new ScpEngineIO("scp -t " /* + (verbose ? "-v " : "") */
    ...
    public class ScpEngineIO {
         ...
         * @param session the session to scp over
         */
        protected ScpEngineIO(String cmd, SshSession session) throws SshException {
            ...
            try {
                ...
                if (!session.executeCommand(cmd)) {
                    session.close();
                    throw new SshException("Failed to execute the command " + cmd, SshException.CHANNEL_FAILURE);
                }
        ...
        /**
         * <p>
         * Write a stream as a file to the remote server. You <strong>must</strong>
         * supply the correct number of bytes that will be written.
         * </p>
         * 
         * @param in stream
         ...
         */
        protected void writeStreamToRemote(InputStream in, long length, String localName, FileTransferProgress progress)
                        throws IOException {
            ...
            while (true) {
                ...
                switch (cmdChar) {
                    ...
                    case 'C':
                        ...
                        return new ScpInputStream(len, in, this, progress, remoteFile) /*
                                                                                         * , 16 *
                                                                                         * 1024)
                                                                                         */
                        ;
        ...
        protected void parseCommand(String cmd, String[] cmdParts) throws IOException {
            ...
            //a command must have the following format "commandtype source dest", and therefore must have 2 spaces
            if ((l == -1) || (r == -1)) {
                ...
                while (count < size) {
                    ...
                    if (progress != null) {
                        ...
                        if (progress.isCancelled())
                            throw new SshIOException(new SshException("SCP transfer was cancelled by user",
    ...
    static class ScpInputStream extends InputStream {
        ...
        public int read(byte[] buf, int off, int len) throws IOException {
            ...
            int r = in.read(buf, off, (int) (length - count > len ? len : length - count));

...
/**
 * This exception is thrown by the {@link SftpSubsystemChannel} class whenever
 * a status message is received that is not expected. This would normally indicate
 * a failure of the operation, for example with a no such file status being returned.
 *
...
public class SftpStatusException extends Exception {
  ...
  // These error codes are not part of the supported versions however are included as some servers are returning them.
  ...
  public SftpStatusException(int status, String msg) {
    super(getStatusText(status) + ": " + msg);
    this.status = status;
  }
